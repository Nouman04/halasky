...
module.exports = {
    ...
    try {
      ...
      fetch(endpoint, requestOptions)
        ...
        .then(async (result) => {
          ...
          let mappedEntertainment = mapAmenities(flightAmenities?.entertainment);
          let mappedFood = mapAmenities(flightAmenities?.food);
  ...
  checkAvailability: async (request, response) => {
    try {
      ...
      let endpoint = `${getSabreUrl()}/v4/shop/flights/revalidate`;
      const myHeaders = new Headers();
      myHeaders.append("Authorization", `Bearer ${accessToken}`);
      myHeaders.append("Content-Type", "application/json");
      myHeaders.append("Accept", "application/json");
  ...
  generatePnr: async (request, response) => {
    try {
      ...
      let endpoint = `${getSabreUrl()}/v2.5.0/passenger/records?mode=create`;
      const myHeaders = new Headers();
      myHeaders.append("Authorization", `Bearer ${accessToken}`);
      myHeaders.append("Content-Type", "application/json");
...
function simplifyFlightResponse(itinerariesList) {
  ...
  return itinerariesList.map((itinerary, index) => {
    ...
    const passengers = itinerary.passengerPriceDetail[0].passengerList.map((passenger) => ({
      ...
      fare: {
        baseFare: `${passenger.passengerTotalFare?.baseFareAmount || 0} ${passenger.passengerTotalFare?.baseFareCurrency || 'N/A'
          }`,
        equivalentFare: `${passenger.passengerTotalFare?.equivalentAmount || 0} ${passenger.passengerTotalFare?.equivalentCurrency || 'N/A'
          }`,
        totalTaxes: `${passenger.passengerTotalFare?.totalTaxAmount || 0} ${passenger.passengerTotalFare?.currency || 'N/A'
          }`,
        totalFare: `${passenger.passengerTotalFare?.totalFare || 0} ${passenger.passengerTotalFare?.currency || 'N/A'
          }`,
      },
      ...
      baggage: passenger.baggageInformation?.map((bag) => ({
        ...
        segment:
          bag.segments[0]?.id >= 0 && itinerary.legList[bag.segments[0].id]?.schedule[0]
            ? `${itinerary.legList[bag.segments[0].id].schedule[0].departure.airport}-${itinerary.legList[bag.segments[0].id].schedule[0].arrival.airport
            }`
            : 'Unknown',
      })) || [],
    ...
    const totalFare = {
      ...
      totalTaxes: `${itinerary.passengerPriceDetail[0].totalFareDetail?.totalTaxAmount || 0} ${itinerary.passengerPriceDetail[0].totalFareDetail?.currency || 'N/A'
        }`,
      totalPrice: `${itinerary.passengerPriceDetail[0].totalFareDetail?.totalPrice || 0} ${itinerary.passengerPriceDetail[0].totalFareDetail?.currency || 'N/A'
        }`,
